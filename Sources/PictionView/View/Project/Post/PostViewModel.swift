...
final class PostViewModel: InjectableViewModel {
    ...
    func build(input: Input) -> Output {
        ...
        let footerInfo = Driver.combineLatest(postItemSuccess, needSubscription)
            .filter { !$1 }
            .flatMap { (postItem, needSubscription) -> Driver<(PostModel, Bool)> in
                return Driver.just((postItem, needSubscription))
            }
